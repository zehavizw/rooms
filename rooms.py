...
def sync_data():
    ...
    if res.status_code != 200:
        st.error(f"שגיאה {res.status_code} בסנכרון.")
        return []
    return res.json()
            ...
            for b in day_bookings:
                ...
                with st.expander(f"⏳ {name} | {start} ({dur} דק')"):
                    ...
                    if st.button("🚀 כניסה", key=f"in_{bid}", use_container_width=True):
                        ...
                        st.rerun()
        else: st.info(f"אין הזמנות ל-{selected_str}")
    else: st.info("לחצי על סנכרון.")

with tab2:
    v_mode = st.radio("תצוגה:", ["⚡ פעילים", "🏁 סיימו"], horizontal=True, key="view_mode_radio")
            ...
            for r in disp:
                ...
                if s_dt.date() == view_date:
                    ...
                    if r.get('status').startswith('active'):
                        ...
                        st.success(f"סיימו. זמן כולל: {int(diff.total_seconds()//60)} דק'")
                    st.divider()
    timer_fragment()
