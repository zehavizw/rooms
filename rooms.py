...
with st.sidebar:
    st.header("כלי מערכת")
    if st.button("🗑️ איפוס זיכרון מלא (Reset)"):
    ...
    if 'web_bookings' in st.session_state:
        for b in st.session_state.web_bookings:
            ...
            with st.expander(f"{status_icon} {name} | {b.get('start_time')} | {scheduled_room}"):
                if is_finished:
                    # הגנה מפני השגיאה: בודקים אם המידע בפורמט הנכון
                    finish_info = st.session_state.finished_bookings.get(bid)
                    room_used = finish_info.get('room_name', 'לא ידוע') if isinstance(finish_info, dict) else 'לא ידוע'
                    
                    st.write(f"✅ האירוח בחדר **{room_used}** הסתיים.")
                    c1, c2 = st.columns(2)
                    if c1.button("🔄 מחדש", key=f"re_{bid}"): del st.session_state.finished_bookings[bid]; st.rerun()
                    if c2.button("➕ המשך", key=f"co_{bid}"): 
                        if isinstance(finish_info, dict):
                            st.session_state.rooms_active[bid] = finish_info
                            del st.session_state.finished_bookings[bid]
                            st.rerun()
                
                elif is_active: 
                    st.info(f"בחדר: **{st.session_state.rooms_active[bid].get('room_name')}**")
                
                else:
                    col_p, col_d = st.columns(2)
                    p = col_p.number_input("אנשים", 1, 50, 2, key=f"p_{bid}")
                    d = col_d.number_input("זמן (דקות)", 15, 300, 60, key=f"d_{bid}")
                    actual_room = st.text_input("לאיזה חדר נכנסים?", value=scheduled_room, key=f"room_edit_{bid}")
                    
                    if st.button("🚀 כניסה", key=f"in_{bid}", use_container_width=True):
                        st.session_state.rooms_active[bid] = {
                            "name": name, "room_name": actual_room, "start": datetime.now(), 
                            "total_people": p, "paying_people": p, "planned_duration": d
                        }
                        send_telegram(f"✅ {name} נכנסו לחדר **{actual_room}**."); st.rerun()
...
with tab3:
    ...
    calc_total = c_col1.number_input("סה\"כ אנשים", 1, 50, 4)
    calc_paying = c_col2.number_input("משלמים", 1, 50, 4)
    calc_minutes = c_col3.number_input("דקות", 1, 600, 60)
    total_res, per_res = calculate_price_logic(calc_total, calc_paying, calc_minutes)
    st.divider()
    res_c1, res_c2 = st.columns(2)
    res_c1.metric("סה\"כ", f"₪{total_res:.2f}")
    res_c2.metric("לאדם", f"₪{per_res:.2f}")
    if st.button("📤 שלח לטלגרם", use_container_width=True):
        msg = f"📝 **בדיקת מחירון: {calc_name}**\n⏰ זמן: {calc_minutes} דק'\n👥 סה\"כ: {calc_total} איש\n💰 סה\"כ: ₪{total_res:.2f}\n👤 לאדם: ₪{per_res:.2f}"
        send_telegram(msg); st.success("נשלח!")
